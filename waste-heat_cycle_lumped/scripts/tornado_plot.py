...
def _build_table(sweep: pd.DataFrame, metric: str, baseline_metric: float) -> pd.DataFrame:
    ...
    for key, grp in sweep.groupby("sweep_param"):
        ...
        if not math.isfinite(metric_base) or metric_base <= 0:
...
def _plot_tornado(table: pd.DataFrame, out_png: Path) -> None:
    ...
    ax.set_xlabel(f"% change in {_METRIC_LABEL} per % change in parameter")
...
def main() -> None:
    ...
    _plot_tornado(table, args.output)
