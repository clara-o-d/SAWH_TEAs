...
%     python analysis/performance/physics/paper_recreation/export_matlab_data.py --figures wilson4
...
assert(isfolder(modelDir), ...
    'Missing model data. Run: python analysis/performance/physics/paper_recreation/export_matlab_data.py --figures wilson4');
...
overlay_ref(axC, refDir, 'Atacama_absorber copy.csv', colAbs, refLabel, 1.5, true);
overlay_ref(axC, refDir, 'Atacama_glass copy.csv', colGlass, '', 1.5, true);
overlay_ref(axC, refDir, 'Atacama_condenser copy.csv', colCond, '', 1.5, true);
...
sgtitle(fig, sprintf([ ...
    ...
    'Model yield = %.3f L/m^2 (measured 0.62 L/m^2),  \\eta_{th} = %.1f%%;  dashed = digitized data'], ...
