...
%     python analysis/performance/physics/paper_recreation/export_matlab_data.py --figures wilson3
...
assert(isfolder(modelDir), ...
    'Missing model data. Run: python analysis/performance/physics/paper_recreation/export_matlab_data.py --figures wilson3');
...
axB = nexttile(tl);
plot_temp_panel(axB, t, mid, lo, hi, colAbs, colGlass, colCond, refDir, refLabel, 'B');
xlim(axB, [0, desHours]);
ylim(axB, [10, 80]);
...
sgtitle(fig, { ...
    ...
    'model = solid; digitized = dashed (temps) / open circles (water); band = h_{amb}=10±2.5 W/m^2K'}, ...
    'FontSize', 11);
...
plot_temp_panel(gca, t, mid, lo, hi, colAbs, colGlass, colCond, refDir, refLabel, '');
xlim(gca, [0, desHours]);
ylim(gca, [10, 80]);
...
function plot_temp_panel(ax, t, mid, lo, hi, colAbs, colGlass, colCond, refDir, refLabel, panelTitle)
hold(ax, 'on');
...
overlay_ref(ax, refDir, 'Cambridge_absorber.csv', colAbs, refLabel, 1.5, true);
overlay_ref(ax, refDir, 'Cambridge_glass.csv', colGlass, '', 1.5, true);
overlay_ref(ax, refDir, 'Cambridge_condenser.csv', colCond, '', 1.5, true);
xlabel(ax, 'time [hr]');
