...
Kinetics are simulated via ``solar_lumped.system.simulate_isothermal_chamber_rh_cycle`` using
the Díaz-Marín hydrogel-only model: Eq. 5 brine thermodynamics (Conde 2004) + Eq. 8 chamber
convection (no Wilson SAWH system, vapor gap, or condenser).
...
from solar_lumped import system as run_solar_sim  # noqa: E402
from chamber_rh_schedule import (  # noqa: E402
    format_schedule_table,
    load_chamber_rh_schedules,
)
from solar_lumped.plotting import (  # noqa: E402
    figure_size_inches,
    panel_size_inches,
...
def simulate_rh_cycle(
    ...
    *,
    ...
    """Delegate to ``solar_lumped.system`` with ESM-inferred RH switch times."""
...
def main() -> Path:
    ...
    print("Díaz-Marín Figure 5 — hydrogel Eq. 5 + 8 (solar_lumped.system)")
    print("=" * 56)
