...
_REPO = Path(__file__).resolve().parent.parent.parent / "waste-heat" / "lumped"
_SRC = _REPO / "src"
if str(_SRC) not in sys.path:
    sys.path.insert(0, str(_SRC))

from waste_heat_lumped.economics import LCOEconomicParams
from waste_heat_lumped.materials import DEFAULT_SALT_MARKER, SALT_MARKERS
from waste_heat_lumped.physics import CANDIDATE_SALTS, get_salt
from waste_heat_lumped.simulation import DeviceConfig
from waste_heat_lumped.simulation import (
    FAIL_LCO,
    passive_gel_temperature_c,
    profile_diagnostics,
    simulate_salt_lcow,
)
from waste_heat_lumped.weather import WeatherClient
from waste_heat_lumped.weather import representative_mean_day_df, site_row_from_hourly
from waste_heat_lumped.weather import profile_from_day_df
...
def main() -> int:
    ...
    p.add_argument(
        ...
        default=1,
        metavar="N",
        help="Warmup daily cycles before the reporting day when cyclic (default: 1; run_waste_heat_sim uses 2).",
    )
    args = p.parse_args()
    ...
    print("=== lcow_random_global_map.py (waste_heat_lumped) ===", flush=True)
    print(
