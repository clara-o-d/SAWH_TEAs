...
(``benchmark/tariff/tariff_results.csv``) to price the water an AWH device would
...
_ROOT = _REPO.parent
_SRC = _REPO / "src"
_BENCHMARK_TARIFF = _ROOT / "benchmark" / "tariff"
for _p in (_SRC, _BENCHMARK_TARIFF):
...
from solar_lumped.economics import HYDROGEL_THICKNESS_M, KG_WATER_PER_M3, LCOEconomicParams  # noqa: E402

_DEFAULT_GRID_CSV = _REPO / "outputs" / "lcow_global" / "lcow_grid_1deg_sites_div1p1.csv"
_DEFAULT_TARIFF_CSV = _BENCHMARK_TARIFF / "tariff_results.csv"
...
def main() -> int:
    ...
    p.add_argument("--tariff-csv", type=Path, default=_DEFAULT_TARIFF_CSV, help="benchmark/tariff/tariff_results.csv")
    p.add_argument(
        "--tariff-tier",
        type=int,
        choices=[6, 15, 50],
