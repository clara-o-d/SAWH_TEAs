...
integration, water-inventory and detailed-plot time series, and annual-yield
aggregation for the two-bed waste-heat SAWH (no HTF loop -- direct waste-heat
coupling).

Consolidated from the former simulation/{device_config, control, coupled_dynamics,
ode_system, water_inventory, detailed_plots, annual_yield}.py. Section headers below
...
from waste_heat_cycle_lumped_no_loop.weather import HalfCycleProfile
...
def compute_controls(
    *,
    ...
    """Vacuum conductance tracks adsorption-limited desorption.
    ...
    """
    del integral_ads_kg_m2, integral_des_kg_m2, t_a_c, t_d_c
    c_vac = clip(
...
class SimulationResult:
    ...
    specific_energy_wh_kwh_per_l: float
    specific_energy_parasitic_kwh_per_l: float
    specific_energy_total_kwh_per_l: float
    n_days: int
...
def simulate_daily(
    ...
    *,
    ...
    from waste_heat_cycle_lumped_no_loop.economics import (
    ...
    wh_kwh_per_l = waste_heat_specific_energy_kwh_per_l(
        thermal_efficiency=eta,
        h_fg_j_per_kg=h_fg,
    )
    parasitic_kwh_per_l = parasitic_specific_energy_kwh_per_l(yield_kg)
    total_kwh_per_l = total_specific_energy_kwh_per_l(
        yield_kg,
        thermal_efficiency=eta,
        h_fg_j_per_kg=h_fg,
    )
    return SimulationResult(
        ...
        specific_energy_total_kwh_per_l=total_kwh_per_l,
        n_days=1,
        n_cycles_per_day=len(results),
    )
