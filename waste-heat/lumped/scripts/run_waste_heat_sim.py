...
from waste_heat_lumped.economics import (
    LcowCostBreakdown,
...
from waste_heat_lumped.physics import inventory_label
...
from waste_heat_lumped.simulation import PhaseResult, run_daily_cycle
from waste_heat_lumped.simulation import (
    plot_water_inventory,
    water_inventory_series,
    write_water_inventory_csv,
)
from waste_heat_lumped.weather import (
...
def register_waste_heat_sim_arguments(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument("--t-f-c", type=float, default=T_F_C, help="Loop fluid setpoint (°C)")
    p.add_argument(
        "--m-dot-f", type=float, default=M_DOT_F_KG_S_M2, help="Loop flow (kg/s/m²)"
    )
    p.add_argument("--ua-gel", type=float, default=UA_GEL_W_K, help="Loop→gel UA (W/K/m²)")
