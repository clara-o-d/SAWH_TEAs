...
Same cost model as ``lcow.py`` (BOM CAPEX, hydrogel replacement, fixed +
parasitic-electricity energy), but CAPEX is paid up front at year 0 instead
of amortized via a capital recovery factor, so year-by-year cash flows can be
discounted directly. Annual OPEX and revenue are held constant across the
device lifetime (no escalation), so NPV reduces to a level-annuity
calculation. Hydrogel-only (this package has no MOF/sorbent switch, unlike
solar_lumped).
"""
...
from waste_heat_cycle_lumped_no_loop.economics.lcow import _hydrogel_cost_per_kg
...
from waste_heat_cycle_lumped_no_loop.economics.params import (
    C_DEVICE_USD,
    KG_WATER_PER_M3,
    LCOEconomicParams,
    dry_composite_mass_kg,
)
from waste_heat_cycle_lumped_no_loop.physics.salt_properties import get_salt_price_usd_per_kg
