...
enters the model \emph{only} through the two Hollands gap correlations -- tilting the device
...
\item \textbf{Maintenance} is a fixed fraction of the bare device BOM cost per year (not the
...
\item \textbf{No energy cost:} the device is passive solar -- no electricity, electric-heat
...
\begin{equation}
C_{annual} = CRF \cdot f_{inv}\,C_{device} + C_{sorbent} + f_{maint}\,C_{device}
\label{eq:annual-cost}
\end{equation}
where $C_{device} = \sum_j C_{BOM,j}$ (six BOM line items),
$f_{inv}$ is the total investment factor, and $f_{maint}$ the maintenance cost fraction.

\textbf{No purchased-energy term:} this device is passive solar, so $C_{annual}$ has no
...
equal; it does not otherwise affect device physics or yield -- \code{lcow\_from\_daily\_yield}
