...
\begin{center}
\small
\texttt{lat, lon, salt, hydrogel\_thickness\_mm, eps\_abs, tau\_glass, fin\_area\_ratio,
vapor\_gap\_mm, warmup\_method, resolution, mean\_yield\_kg\_m2, mean\_eta\_thermal, n\_periods}
\end{center}

This is water yield and thermal efficiency only -- no LCOW or economics. That can be computed
from yield as a separate downstream step, the same way \texttt{lcow\_full\_global\_map.py} does.

\section{Smoke test (already run locally)}

Before any Sherlock submission, the following was validated on this machine:
...
\subsection{Sherlock-side smoke test (to run there)}
The only thing not yet validated is that this behaves identically on the cluster. A single-task
array reproducing the exact run above:

\begin{verbatim}
ml python/3.12.1 uv
uv venv && source .venv/bin/activate
uv pip install numpy scipy pandas requests-cache retry-requests shapely cartopy
...
python scripts/grid_param_sweep.py \
  ...
  --hydrogel-thickness-mm 4.0 --eps-abs 0.90 0.95 \
  --tau-glass 0.85 --fin-area-ratio 7.1 \
  --output-csv outputs/smoke_test_sherlock.csv
EOF
\end{verbatim}

Expected: two rows, yields matching \num{1.707476} / \SI{1.800478}{\kilo\gram\per\meter\squared}
(exactly, since this is deterministic given the same cached weather and the same code), total
runtime around \SI{12}{\minute} (matches the local measurement above; scale \texttt{--time} up
if the cluster's per-core throughput is slower). \textbf{This step needs the weather already
cached} -- see Section~\ref{sec:weather} -- otherwise it will additionally pay one Open-Meteo
fetch.
...
per-combo cost (Section~\ref{sec:cost}), one site takes 14.2--49.6 \emph{hours} serially, far
beyond any reasonable single-task \texttt{--time}. Each site's 135 combos are instead split into
15 chunks of 9 combos (135 divides evenly), so each array task is one (site, chunk) pair costing
roughly \SI{0.95}{}--\SI{3.31}{\hour}. \texttt{--combo-offset}/\texttt{--combo-limit} (added to
\texttt{grid\_param\_sweep.py} for this) slice the site's combo grid; the two array-index
components are recovered from a single flattened SLURM index via integer div/mod:

\begin{verbatim}
#!/bin/bash
#SBATCH --job-name=sawh-sweep
#SBATCH --array=0-21074%300
#SBATCH --time=06:00:00
#SBATCH --cpus-per-task=1
#SBATCH --mem-per-cpu=4G
#SBATCH --partition=owners

N_CHUNKS_PER_SITE=15
CHUNK_SIZE=9
...
python scripts/grid_param_sweep.py \
  ...
  --cache-dir $GROUP_HOME/.weather_cache \
...
whole chunk. \texttt{--time=06:00:00} leaves nearly $2\times$ margin over the slowest measured
chunk (\SI{3.31}{\hour}); tighten it once more of the grid's actual climate spread is known.

\textbf{Check the array size against this cluster's limit before submitting}: 21,075 elements in
one \texttt{--array} directive may exceed the configured \texttt{MaxArraySize} (commonly a few
thousand by default). Check with \texttt{scontrol show config | grep -i MaxArraySize} on a login
node; if 21,075 exceeds it, split the submission into several \texttt{sbatch} calls each covering
a slice of \texttt{SLURM\_ARRAY\_TASK\_ID} (e.g. \texttt{--array=0-9999}, \texttt{--array=10000-19999},
\texttt{--array=20000-21074}) rather than one call.
...
TTL is fixed -- see repository history). 1405 concurrent Sherlock nodes must \emph{not} each
independently hit the API: prefetch/cache all 1405 sites in one throttled pass first (locally, or
from a single Sherlock node), then ship the resulting \texttt{.weather\_cache} directory to
\texttt{\$GROUP\_HOME} before submitting the array. The full sweep should then make zero network
calls.

Use \texttt{scripts/prefetch\_weather.py}, not a shell loop calling
\texttt{grid\_param\_sweep.py} once per site: building the land/country-exclusion geometry and
importing the physics stack costs $\sim$\SI{1.4}{\second} and is identical for every site, but a
subprocess-per-site loop pays that cost 1405 times. \texttt{prefetch\_weather.py} pays it once
and fetches every site in the same process and connection pool -- measured $\sim$6$\times$
faster overall (a 1405-site run drops from $\sim$30+ minutes to $\sim$5--10 minutes):

\begin{verbatim}
python scripts/prefetch_weather.py --step 3
\end{verbatim}

If running this on Sherlock itself rather than locally, use an interactive \texttt{sh\_dev}
session rather than the login node (it's light on CPU but runs for several minutes, which is
what \texttt{sh\_dev} is for).

\subsection{Merging outputs}\label{sec:merge}
The array produces up to 21,075 separate CSV files (one per (site, chunk), all sharing the same
...
echo "found $n of 21075 expected chunk files"
...
Measured per-combo cost at \texttt{--resolution monthly} (post 2-cycle fix): \SI{380.0}{\second}
(Atacama desert, low-seasonal-variance) to \SI{1323.4}{\second} (Cambridge, moderate-seasonal-
variance) -- roughly a $3.5\times$ spread by climate, consistent with everything else observed
in this project. Total: $189{,}675$ units (1405 sites $\times$ 135 combos).
...
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Cost basis} & \textbf{Core-hours} & \textbf{@200 cores} & \textbf{@500 cores} & \textbf{@1000 cores} \\
\midrule
Fast/typical (\SI{380.0}{\second}/combo) & 20{,}021 & 100.1 h & 40.0 h & 20.0 h \\
Slow (\SI{1323.4}{\second}/combo) & 69{,}727 & 348.6 h & 139.5 h & 69.7 h \\
\bottomrule
\end{tabular}
\end{table}

So: realistically \textbf{20{,}000--70{,}000 core-hours}, i.e. roughly 1--3 days at 1000
concurrent cores, or under 6 days at 500 cores in the typical (non-Cambridge-like) case. Dropping
vapor gap from the sweep (Section~\ref{sec:swept}, $405\to135$ combos) and excluding northern
Canada/Russia (Section~\ref{sec:fixed-above}, $1635\to1405$ sites) already cut this from what it
