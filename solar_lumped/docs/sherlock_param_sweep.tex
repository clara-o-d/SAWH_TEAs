...
independently hit the API: prefetch/cache all 1405 sites in one throttled pass first (locally,
reusing what's already cached, or from a single Sherlock login/dev node), then ship the resulting
\texttt{.weather\_cache} directory to \texttt{\$GROUP\_HOME} before submitting the array. The
full sweep should then make zero network calls. \texttt{--combo-limit 0} runs the weather
fetch and builds the monthly profiles but evaluates no combos (confirmed: $\sim$\SI{1}{\second}
per site once cached), making it a free prefetch-only invocation with no extra tooling needed:

\begin{verbatim}
for i in $(seq 0 1404); do
  python scripts/grid_param_sweep.py --site-index $i --step 3 --combo-limit 0 \
    --output-csv /dev/null
  sleep 0.5
done
\end{verbatim}
